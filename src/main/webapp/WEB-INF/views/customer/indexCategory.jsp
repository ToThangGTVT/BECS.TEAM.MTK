                        ...
                        <aside class="wedget__categories poroduct--cat">
                            ...
                            <ul>
                                <c:forEach var="category" items="${categoryList}">
                                    <li><a href="<%=request.getContextPath()%>/category/${category.category.name}-${category.category.id}">${category.category.name}<span>(${category.count})</span></a></li>
                                </c:forEach>
                            </ul>
                        </aside>
                        </aside>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div
                                ...
                                <p>Showing 1–${bookList.size()} of ${countBook} results for "${category.name}"</p>
                        ...
                        <div class="shop-grid tab-pane fade show active" id="nav-grid" role="tabpanel">
                                    ...
                                    <div class="product product__style--3 col-lg-4 col-md-4 col-sm-6 col-12">
                                        <div class="product__thumb" style="max-height: 220px; height: 220px">
                                            <a class="first__img" href="single-product.jsp"><img
                                                    style="width: 230px"
                                                    src="<%=request.getContextPath()%>/resources/upload/${book.image}"></a>
                                        </div>
                            ...
                            <nav aria-label="Page navigation example">
                                <ul class="pagination justify-content-center">
                                    <c:if test="${pageNum > 0}">
                                        <li class="page-item text-success"><a class="page-link" href="?page=1">First</a></li>
                                    </c:if>
                                    <c:if test="${pageNum + 1 < countBook/12+1}">
                                        <li class="page-item disabled"><a class="page-link" href="?page=${pageNum}">${pageNum}</a></li>
                                    </c:if>
                                    <c:if test="${pageNum + 1 < countBook/12+1}">
                                        <li class="page-item"><a class="page-link" href="?page=${pageNum+1}">${pageNum+1}</a></li>
                                    </c:if>
                                    <c:if test="${pageNum+2 < countBook/12+1}">
                                        <li class="page-item"><a class="page-link" href="?page=${pageNum+2}">${pageNum+2}</a></li>
                                    </c:if>
                                    <c:if test="${pageNum+3 < countBook/12+1}">
                                        <li class="page-item"><a class="page-link" href="?page=${pageNum+3}">${pageNum+3}</a></li>
                                    </c:if>
                                    <c:if test="${pageNum > 0}">
                                        <li class="page-item"><a class="page-link" href="?page=<fmt:formatNumber value="${countBook/12+1}" type="number" pattern="#"/>">End</a></li>
                                    </c:if>
                                </ul>
                            </nav>
...
<script>
    $('#myModal').on("show.bs.modal", function (e) {
        ...
        $.getJSON({
            type: "POST",
            url: "<%=request.getContextPath()%>/api/info/book",
            data: {
                id: recipient
            },
